...
function get_images()
	...
	for i, v in ipairs(images) do
		...
		data[index]["_id"] = '<a href="javascript:new_tag(\''..v.Id:sub(8,20)..'\')" class="podmaner-link" title="'..translate("New tag")..'">' .. v.Id:sub(8,20) .. '</a>'

		if v.RepoTags and next(v.RepoTags)~=nil then
			for i, v1 in ipairs(v.RepoTags) do
				data[index]["_tags"] =(data[index]["_tags"] and ( data[index]["_tags"] .. "<br />" )or "") .. ((v1:match("<none>") or (#v.RepoTags == 1)) and v1 or ('<a href="javascript:un_tag(\''..v1..'\')" class="podmaner_link" title="'..translate("Remove tag")..'" >' .. v1 .. '</a>'))
		...
		for ci,cv in ipairs(containers) do
			if v.Id == cv.ImageID then
				...
				'<a href='..luci.dispatcher.build_url("admin/docker/container/"..cv.Id)..' class="podmaner_link" title="'..translate("Container detail")..'">'.. cv.Names[1]:sub(2).."</a>"
			end
		end
...
o.template = "podmaner/cbi/inlinevalue"
...
o.template = "podmaner/cbi/inlinebutton"
o.inputstyle = "add"
...
o.template = "podmaner/images_import"
...
s = m:section(SimpleSection)
s.template = "podmaner/apply_widget"
...
o.template = "podmaner/images_load"
o.inputstyle = "add"

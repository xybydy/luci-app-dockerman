...
Copyright 2019 lisaac <https://github.com/lisaac/luci-app-podmaner>
]]--

local docker = require "luci.model.docker"

module("luci.controller.podmaner",package.seeall)

function index()
	entry({"admin", "docker"},
		firstchild(),
		_("Docker"),
		40).acl_depends = { "luci-app-podmaner" }

	entry({"admin", "docker", "config"},cbi("podmaner/configuration"),_("Configuration"), 1).leaf=true
	...
	entry({"admin", "docker", "overview"}, form("podmaner/overview"),_("Overview"), 2).leaf=true
	entry({"admin", "docker", "containers"}, form("podmaner/containers"), _("Containers"), 3).leaf=true
	entry({"admin", "docker", "images"}, form("podmaner/images"), _("Images"), 4).leaf=true
	entry({"admin", "docker", "networks"}, form("podmaner/networks"), _("Networks"), 5).leaf=true
	entry({"admin", "docker", "volumes"}, form("podmaner/volumes"), _("Volumes"), 6).leaf=true
	entry({"admin", "docker", "events"}, call("action_events"), _("Events"), 7)

	entry({"admin", "docker", "newcontainer"}, form("podmaner/newcontainer")).leaf=true
	entry({"admin", "docker", "newnetwork"}, form("podmaner/newnetwork")).leaf=true
	entry({"admin", "docker", "container"}, form("podmaner/container")).leaf=true
...
function action_events()
	...
	luci.template.render("podmaner/logs", {self={syslog = logs, title="Events"}})
end
